...
public class SecureBulkLoadEndpoint extends BaseEndpointCoprocessor
  ...
  public void cleanupBulkLoad(String bulkToken) throws IOException {
    ...
    fs.delete(createStagingDir(baseStagingDir, getActiveUser(),
        env.getRegion().getTableDesc().getName(), new Path(bulkToken).getName()), true);
  }
      ...
      loaded = ugi.doAs(new PrivilegedAction<Boolean>() {
        ...
        public Boolean run() {
          ...
          try {
            ...
            LOG.error("Failed to secure bulk load", e);
          } catch (Exception e) {
  ...
  public static class SecureBulkLoadListener implements HRegion.BulkLoadListener {
    ...
    public void failedBulkLoad(final byte[] family, final String srcPath) throws IOException {
      ...
      LOG.debug("Moving Back " + stageP + " to " + p);
